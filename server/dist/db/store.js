...
export function isBarOpenNow(bar, date = new Date()) {
...
class Store {
    ...
    init() {
        ...
        if (fs.existsSync(DB_FILE)) {
            try {
                ...
                if (!this.data.bars || this.data.bars.length === 0)
                    this.data.bars = [...seedBars];
                if (!this.data.reviews || this.data.reviews.length === 0)
                    this.data.reviews = [...seedReviews];
                if (!this.data.crawls || this.data.crawls.length === 0)
                    this.data.crawls = [...seedCrawls];
                ...
                if (!this.data.events || this.data.events.length === 0)
                    this.data.events = [...seedEvents];
                if (!this.data.userProfiles)
    ...
    getBars(filters) {
        ...
        if (filters.q && filters.q.trim() !== '') {
            ...
            results = results.filter((b) => b.name.toLowerCase().includes(term) ||
    ...
    addReview(barId, reviewData) {
        const bar = this.data.bars.find((b) => b.id === barId);
        if (!bar)
            throw new Error('Bar introuvable');
        const newReview = {
    ...
    getUserProfile(userId = 'default-user') {
    ...
    toggleFavorite(userId = 'default-user', barId) {
    ...
    toggleVisited(userId = 'default-user', barId) {
        const user = this.getUserProfile(userId);
    ...
    toggleBucketList(userId = 'default-user', barId) {
        const user = this.getUserProfile(userId);
    ...
    getBadgesWithProgress(userId = 'default-user') {
        const user = this.getUserProfile(userId);
        ...
        const eventsCreatedCount = allEvents.filter((e) => e.organizer && (e.organizer.name === user.name || e.organizer.id === userId)).length;
    ...
    addImportedBars(bars) {
        let added = 0;
        let skipped = 0;
        for (const b of bars) {
            const alreadyExists = this.data.bars.some((existing) => existing.name.toLowerCase() === b.name.toLowerCase() &&
