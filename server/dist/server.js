...
app.get('/api/bars', (req, res) => {
    try {
        const { q, city, country, type, types, priceLevel, priceLevels, minRating, openNow, terrace, food, happyHour, liveMusic, vibe, lat, lng, radiusKm, north, south, east, west, } = req.query;
...
app.get('/api/bars/random', (req, res) => {
    try {
...
app.post('/api/bars', (req, res) => {
    try {
        const { name, tagline, description, type, address, city, country, lat, lng, priceLevel, photos, coverPhoto, specialties, vibes, amenities, phone, website, hours, submittedBy, } = req.body;
        ...
        const newBar = store.addBar({
            ...
            lat: Number(lat),
            lng: Number(lng),
            priceLevel: Math.min(4, Math.max(1, Number(priceLevel) || 2)),
...
app.post('/api/bars/:id/reviews', (req, res) => {
    try {
...
app.post('/api/bars/:id/report', (req, res) => {
    try {
...
app.get('/api/crawls', (req, res) => {
    try {
        ...
        const crawls = store.getCrawls(city);
        res.json(crawls);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/user/visited', (req, res) => {
    try {
        const { barId } = req.body;
        if (!barId)
            return res.status(400).json({ error: 'barId requis' });
...
app.post('/api/user/bucketlist', (req, res) => {
    try {
        const { barId } = req.body;
        if (!barId)
            return res.status(400).json({ error: 'barId requis' });
...
app.post('/api/import/osm', async (req, res) => {
    ...
    `;
        ...
        const importedBars = elements
            .filter((el) => el.tags && el.tags.name)
            .map((el, index) => {
            const tags = el.tags;
            let type = 'pub';
            if (tags.amenity === 'bar' || tags.cuisine === 'cocktail')
                type = 'cocktail';
            if (tags.brewery || tags.microbrewery)
                type = 'brewery';
            if (tags.rooftop === 'yes')
                type = 'rooftop';
            if (tags.wine)
